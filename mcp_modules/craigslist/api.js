...
export async function searchCity(city, options) {
  try {
    ...
    // If direct search failed or returned no results, log and return empty array
    logger.warn(
      `Direct search failed or returned no results for ${searchUrl}, continuing to next query`
    );
    return [];
  } catch (error) {
